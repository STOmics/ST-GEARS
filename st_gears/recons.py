...
def stack_slices_pairwise_rigid(
        ...
        label_col: str,
        fil_pc = 20,
        filter_by_label: bool = True) -> Tuple[List[AnnData]]:
    """
    ...
    spaA, spaB = filter_rows_cols_to_spa(slicesl[0], slicesl[1], filter_by_label, label_col, spaAtype='spatial', spaBtype='spatial')
    ...
    try:
        ...
        slicesl[0].obsm['spatial_rigid'] = slicesl[0].obsm['spatial'] - np.broadcast_to(tA, slicesl[0].obsm['spatial'].shape)
        slicesl[1].obsm['spatial_rigid'] = slicesl[1].obsm['spatial'] - np.broadcast_to(tB, slicesl[1].obsm['spatial'].shape)
    ...
    for i in range(1, len(slicesl) - 1):
        ...
        spaA, spaB = filter_rows_cols_to_spa(slicesl[i], slicesl[i+1], filter_by_label, label_col, spaAtype='spatial_rigid', spaBtype='spatial')
        ...
        try:
            ...
            slicesl[i+1].obsm['spatial_rigid'] = slicesl[i+1].obsm['spatial'] - np.broadcast_to(tB, slicesl[i+1].obsm['spatial'].shape)
    ...
    for i, slice in enumerate(slicesl):
        slice.obsm['spatial_rigid'][:, 2] = slice.obsm['spatial'][:, 2]
...
def stack_slices_pairwise_elas(
        ...
        lambda_val: type(None)=None) -> Tuple[List[AnnData]]:
    ...
    for i in range(len(slicesl) - 1):
        ...
        try:
            ...
            print('transition matrix didn\'t work for the interpolating elastic method')
...
def stack_slices_pairwise_elas_field(
        ...
        fil_pc: float = 20,
        filter_by_label: bool = True,
        sigma: float=1) -> Tuple[List[AnnData]]:
    """
    ...
    Args:
        ...
        pis: list of probabilistic transition matrix
        label_col:  column name of adata.obs that stores annotated celltypes
        pixel_size: edge length of single pixel, when generating elastic field
        fil_pc: percentage of ranked probabilities in transition matrix, after which probabilities will be filtered.
        filter_by_label: if spots were filtered acordding to annotation, in the ot solving process, or not
        warp_type: 'linear', 'cubic', 'tps'
        sigma: sigma value of gaussina kernel, when filtering noises in elastic registration field. Refer to this
               website to decide sigma according to your desired range of convolution. http://demofox.org/gauss.html
    Returns:
        elastic registration result saved in adata.obsm['spatial_elas']
    """

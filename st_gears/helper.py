...
def determine_using_gpu(use_gpu):
    ...
    if not use_gpu:
        ...
        try:
            ...
            print("We currently only have GPU support for torch. Please install torch before assigning GPU using, or assign use_gpu as False to use CPU")
            quit()

        if not torch.cuda.is_available():
            print("torch.cuda is not available. Please make sure cuda is installed, or assign use_gpu as False to use CPU")
            quit()
        else:
            use_gpu = True
            return use_gpu

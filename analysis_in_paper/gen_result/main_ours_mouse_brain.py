...
fig_dir = '/storeData/USER/data/xiatianyi/paper_gears/ouput_pro_fig/mouse_brain_ours_nonuniform'
out_dir = '/storeData/USER/data/xiatianyi/paper_gears/output_data/mouse_brain_ours_nonuniform'
...
for ctype in ['cell_label', 'num_anchors', 'pi_max', 'strong_anch', 'weight', 'sum_gene']:
    for spatype in ['spatial', 'spatial_rigid']:
        rp.plot_scatter_by_grid(slicesl, anncell_cid, ali=ali, bli=bli, pili=pili, tyscoreli=tyscoreli, alphali=alphali, figsize=(12.5, 2.5),  # 1.25 for each row
...
slicesl = recons.stack_slices_pairwise_elas_field([slicesl[i] for i in regis_ilist], pili, label_col='annotation', pixel_size=200, fil_pc=20, filter_by_label=filter_by_label, sigma=1)
